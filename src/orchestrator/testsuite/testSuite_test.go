...
var templateFromFile = `[{"test": %s,"rampTime": "%s","repeatTime": "%s","stopAllLorhammerTime": "%s","sleepBeforeCheckTime": "%s","shutdownAllLorhammerTime": "%s","sleepAtEndTime": "%s","init": %s,"provisioning": %s,"check": %s, "deploy": %s}]`

func TestTransformFile(t *testing.T) {
	for _, ct := range testsFromFile {
		data := []byte(fmt.Sprintf(templateFromFile, ct.test, ct.rampTime, ct.repeatTime, ct.stopAll, ct.beforeCheck, ct.shutdownAll, ct.sleep, ct.init, ct.provisioning, ct.check, ct.deploy))
