...
var templateLaunch = `[{"test": %s,"rampTime": "%s","repeatTime": "%s","stopAllLorhammerTime": "%s","sleepBeforeCheckTime": "%s","shutdownAllLorhammerTime": "%s","sleepAtEndTime": "%s","requieredLorhammer":%d,"maxWaitLorhammeTime":"%s","init": %s,"provisioning": %s,"check": %s, "deploy": %s}]`
...
func TestLaunchTest(t *testing.T) {
	...
	for _, test := range testsLaunch {
		t.Run(test.description, func(t *testing.T) {
			var ct = test
			data := []byte(fmt.Sprintf(templateLaunch, ct.test, ct.rampTime, ct.repeatTime, ct.stopAll, ct.beforeCheck, ct.shutdownAll, ct.sleep, ct.requieredLorhammer, ct.maxWaitLorhammeTime, ct.init, ct.provisioning, ct.check, ct.deploy))
